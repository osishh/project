...
def define_G(input_nc, output_nc, ngf, which_model_netG, norm='batch', use_dropout=False, gpu_ids=[], skip=False, opt=None):
    ...
    # TODO IMDB
    elif which_model_netG == 'IMDB_sid_unet_resize':
        netG = IMDB_Unet_resize_conv(opt, skip)
    else:
        raise NotImplementedError('Generator model name [%s] is not recognized' % which_model_netG)
    if len(gpu_ids) >= 0:
        if len(gpu_ids) > 0 and torch.cuda.is_available():
            netG.cuda(device=gpu_ids[0])  # Use GPU if available
        else:
            netG.cpu()  # Use CPU mode
...
def define_D(input_nc, ndf, which_model_netD,
    ...
    elif which_model_netD == 'featured_layers':
        netD = FeatureChoicedNLayerDiscriminator(input_nc, ndf, n_layers_D, norm_layer=norm_layer, use_sigmoid=use_sigmoid, gpu_ids=gpu_ids)
...
class FeatureSelector(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.S1 = nn.Sequential(*sequence1)
        sequence2 = [
            nn.Conv2d(in_channels, out_channels, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(out_channels),
            nn.ReLU(inplace=True),
        ...
        self.S2 = nn.Sequential(*sequence2)
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        sequence3 = [
            nn.Linear(out_channels, out_channels),
            nn.Linear(out_channels, out_channels),
        ]
        self.S3 = nn.Sequential(*sequence3)
        
    def forward(self, x):
        identity = x
        x = self.S1(x) + identity
        identity = x
        x = self.S2(x) + identity
        x = self.avgpool(x)
        x = x.view(x.size(0), -1)
        x = self.S3(x)
...
class FeatureChoicedNLayerDiscriminator(nn.Module):
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d, use_sigmoid=False, gpu_ids=[]):
        super(FeatureChoicedNLayerDiscriminator, self).__init__()
        self.gpu_ids = gpu_ids

        kw = 4
        padw = int(np.ceil((kw-1)/2))
        self.feature = FeatureSelector(input_nc, input_nc)
        ...
        self.S1 = nn.Sequential(*sequence)
        
        # self.FeatureSelector = FeatureSelector(ndf * nf_mult, ndf * nf_mult)
        sequenceAfter = [nn.Conv2d(ndf * nf_mult, 1, kernel_size=kw, stride=1, padding=padw)]
        # sequence += [nn.Conv2d(ndf * nf_mult, 1, kernel_size=kw, stride=1, padding=padw)]

        if use_sigmoid:
            sequenceAfter += [nn.Sigmoid()]

        self.S2 = nn.Sequential(*sequenceAfter)
        # self.model = nn.Sequential(*sequence)

    def forward(self, input):
        # if len(self.gpu_ids) and isinstance(input.data, torch.cuda.FloatTensor):
        #     module = nn.parallel.data_parallel(self.model,input)
        #     return module(input)
        # else:
        feature = self.feature(input)
        # 拓展feature的维度
        fused_feature = feature.unsqueeze(2).unsqueeze(3).expand_as(input)
        fused_feature = input + fused_feature
        prefeature = self.S1(fused_feature)
        return self.S2(prefeature)
...
class Unet_resize_conv(nn.Module):
    ...
    def forward(self, input, gray):
        ...
        if self.skip:
            ...
            return output
class IMDB_Unet_resize_conv(nn.Module):
    def __init__(self, opt, skip):
        super(IMDB_Unet_resize_conv, self).__init__()
        self.opt = opt
        self.skip = skip
        p = 1
        # self.conv1_1 = nn.Conv2d(4, 32, 3, padding=p)
    ...
    def forward(self, input, gray):
        ...
        if self.opt.use_norm == 1:
            ...
            x = self.LReLU5_1(self.conv5_1(x))
            x = x*gray_5 if self.opt.self_attention else x
            # x = x*gray_imdb if self.opt.self_attention else x
            conv5 = self.LReLU5_2(self.conv5_2(x))
